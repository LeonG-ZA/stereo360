...
Convert monoscopic 360° equirectangular video into stereoscopic top-bottom
360° video suitable for VR headsets.

> Status: **M3 — temporal consistency**. Default pipeline: per-frame depth
> (Depth Anything V2) + edge-aware guided-filter smoothing + DIBR warping.
> For flicker-free output, `--depth-backend video-depth-anything` uses the
> temporal Video Depth Anything model with chunked overlap blending. See
...
- Desktop UI: `pip install -r requirements-ui.txt`
- ONNX backend: `pip install -r requirements-onnx.txt` (running an exported
  model needs onnxruntime only; `scripts/export_onnx.py` also needs onnx and
  onnxscript)
- Temporal depth backend: `pip install -r requirements-vda.txt`
  (`--depth-backend video-depth-anything` only)

Each optional backend keeps its own requirements file rather than adding to
everyone's install. Selecting one without its packages now reports which file
to install instead of a bare `ModuleNotFoundError`.
